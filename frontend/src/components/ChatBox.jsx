...
export default function ChatBox({ messages, onSendMessage, isLoading, onPlayTimestamp }) {
  ...
  return (
    <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      {/* Header */}
      <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid var(--border-light)', backgroundColor: 'rgba(15, 23, 42, 0.4)' }}>
        <h3 style={{ fontSize: '1.1rem', margin: 0 }}>Document Q&A</h3>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        {messages.length === 0 ? (
          <div style={{ margin: 'auto', textAlign: 'center', color: 'var(--text-muted)' }}>
            <Bot size={48} style={{ opacity: 0.2, margin: '0 auto 1rem auto' }} />
          ...
          messages.map((msg, idx) => (
            <div key={idx} className="animate-fade-in" style={{ display: 'flex', gap: '1rem', flexDirection: msg.role === 'user' ? 'row-reverse' : 'row' }}>
              ...
              <div style={{ 
                backgroundColor: msg.role === 'user' ? 'var(--bg-tertiary)' : 'rgba(15, 23, 42, 0.5)',
                ...
                <p style={{ margin: 0, whiteSpace: 'pre-wrap', fontSize: '0.95rem' }}>{msg.content}</p>
                
                {/* Play Timestamp Button - Only for AI messages with a timestamp */}
        ...
        {isLoading && (
          <div className="animate-fade-in" style={{ display: 'flex', gap: '1rem' }}>
            ...
            <div style={{ backgroundColor: 'rgba(15, 23, 42, 0.5)', padding: '1rem 1.25rem', borderRadius: '0 var(--radius-lg) var(--radius-lg) var(--radius-lg)', border: '1px solid var(--border-light)', display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
      ...
      {/* Input Area */}
      <div style={{ padding: '1rem 1.5rem', borderTop: '1px solid var(--border-light)', backgroundColor: 'rgba(15, 23, 42, 0.4)' }}>
        <form onSubmit={handleSubmit} style={{ position: 'relative' }}>
          ...
          <button 
            ...
            style={{ 
              ...
              backgroundColor: input.trim() && !isLoading ? 'var(--accent-primary)' : 'transparent',
